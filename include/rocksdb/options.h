...
struct DBOptions {
  ...
  // Any internal progress/error information generated by the db will
  // be written to trace_log if it is non-nullptr, or to a file stored
  // in the same directory as the DB contents if trace_log is nullptr.
  // Default: nullptr
  std::shared_ptr<Logger> trace_log = nullptr;
...
extern Status CreateTracerFromOptions(const std::string& dbname,
                                      const DBOptions& options,
                                      std::shared_ptr<Logger>* tracer);
