...
Status TraceReader::ReadHeader() {
  ...
  if (header.payload.substr(0, 16) != "feedcafedeadbeef") {
...
Status TraceReader::ReadRecord(Trace& trace) {
  ...
  if (result_.size() < 8) {
    return Status::Corruption("The trace file is corrupted. while reading timestamp");
  }
  offset_ += 8;
  ...
  offset_ += 1;
  if (result_.size() < 1) {
    return Status::Corruption("The trace file is corrupted. while reading trace type");
  }
  ...
  offset_ += 4;
  if (result_.size() < 4) {
    return Status::Corruption("The trace file is corrupted. while reading payload length");
  }
  ...
  while (to_read > 0) {
    ...
    if (result_.size() < to_read) {
      return Status::Corruption("The trace file is corrupted. while reading payload");
    }
